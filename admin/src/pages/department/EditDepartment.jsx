...
import { Box, Button, Dialog, Divider, Grid, IconButton, MenuItem, Slide, TextField, Typography } from '@mui/material';
...
const EditDepartment = ({ open, handleClose }) => {
    ...
    const [credentials, setCredentials] = useState({
        ...
        categoryName: values.categoryName?._id,
        cat: {
            label: values.categoryName?.name,
            value: values.categoryName?._id, },
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        const res = await axios.put(`/v1/department/${values._id}`, credentials, configOptions)
        if (res.data.success) {
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                <Select
                                    ...
                                    onChange={(e)=> {
                                        handleChange(e)
                                        setCredentials(prev => ({ ...prev, cat: e }))
                                        }}
                                />
                            </Grid>
                            <Grid item xs={12}>
